...
int test_mult_identity(int M, int N, int lb, int verbose)
{
  ...
  conf.lb = lb;
  wsize = matrix_lqmult_work(&C, ARMAS_LEFT, &conf);
  ...
  conf.error = 0;
  matrix_lqmult(&C, &A0, &tau0, &W, ARMAS_LEFT, &conf);
  matrix_lqmult(&C, &A0, &tau0, &W, ARMAS_LEFT|ARMAS_TRANS, &conf);
...
int test_mult_left(int M, int N, int lb, int verbose)
{
  ...
  // allocate workspace according the blocked multiplication
  conf.lb = lb;
  wsize = matrix_lqmult_work(&C0, ARMAS_LEFT, &conf);
  ...
  conf.error = 0;
  // compute C0 = Q.T*Q*C0
  matrix_lqmult(&C0, &A0, &tau0, &W, ARMAS_LEFT, &conf);
  matrix_lqmult(&C0, &A0, &tau0, &W, ARMAS_LEFT|ARMAS_TRANS, &conf);

  n0 = rel_error((__Dtype *)0, &C0, &C1, ARMAS_NORM_ONE, ARMAS_NONE, &conf);
...
int test_mult_right(int M, int N, int lb, int verbose)
{
  ...
  // allocate workspace according the blocked multiplication
  conf.lb = lb;
  wsize = matrix_lqmult_work(&C0, ARMAS_RIGHT, &conf);
  matrix_init(&W, wsize, 1);
  ...
  // compute C0 = C0*Q.T*Q
  conf.error = 0;
  matrix_lqmult(&C0, &A0, &tau0, &W, ARMAS_RIGHT|ARMAS_TRANS, &conf);
  matrix_lqmult(&C0, &A0, &tau0, &W, ARMAS_RIGHT, &conf);
