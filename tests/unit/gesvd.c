...
int test1(int M, int N, int verbose)
{
    ...
    armas_d_mcopy(&A0, &A);
    nrm_A = armas_d_mnorm(&A, ARMAS_NORM_ONE, &conf);

    nwrk = armas_d_svd_work(&A, ARMAS_WANTU|ARMAS_WANTV, &conf);
    armas_d_init(&W, nwrk, 1);
    ...
    n2 = armas_d_mnorm(&A0, ARMAS_NORM_ONE, &conf) / nrm_A;
