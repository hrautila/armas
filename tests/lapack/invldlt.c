...
int test_ldlnp_inv(int N, int lb, int flags, int verbose)
{
    ...
    armas_ldlinverse(&A0, ARMAS_NOPIVOT, flags, &conf);

    armas_mult_sym(ZERO, &C, ONE, &A0, &A1, flags | ARMAS_LEFT, &conf);
...
int test_ldl_inv(int N, int lb, int flags, int verbose)
{
    ...
    armas_ldlinverse(&A0, &P0, flags, &conf);

    armas_mult_sym(0.0, &C, 1.0, &A0, &A1, flags | ARMAS_LEFT, &conf);
