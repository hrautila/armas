...
int set_diagonals(armas_dense_t * A, int flags, int type)
{
    armas_dense_t sD, sE;
    ...
    armas_init(&sE, 0, 0);
    armas_diag(&sD, A, 0);
    armas_diag(&sE, A, (flags & ARMAS_LOWER ? -1 : 1));
    N = armas_size(&sD);
    ...
    case 2:
        ...
        for (k = 0; k < N - 1; k++) {
            if (k & 0x1) {
                armas_set_at(&sD, N - 1 - E, k + 1);
                E++;
            } else {
                armas_set_at(&sD, S, k + 1);
                S++;
            }
            if (k < N - 1)
                armas_set_at(&sE, k, 1.0);
        }
        break;
    case 1:
        // bidiagonal larger values on top
        for (k = 0; k < N - 1; k++) {
            armas_set_at(&sD, N - 1 - k, k + 1);
            if (k < N - 1)
                armas_set_at(&sE, k, 1.0);
        }
        break;
...
{
    armas_dense_t A0, At, U, V, D, E, sD, sE, C;
    ...
    armas_submatrix(&At, &A0, 0, 0, N, N);

    armas_init(&D, N, 1);
    armas_init(&E, N - 1, 1);
    armas_diag(&sD, &A0, 0);
    armas_diag(&sE, &A0, (flags & ARMAS_LOWER) ? -1 : 1);
    ...
    //armas_bdsvd_w(&D, &E, &U, &V, flags|ARMAS_WANTU|ARMAS_WANTV, &wb, &conf);
    armas_bdsvd(&D, &E, &U, &V, flags | ARMAS_WANTU | ARMAS_WANTV, &conf);

    // compute: U.T*A*V
    armas_mult(ZERO, &C, ONE, &U, &A0, ARMAS_TRANSA, &conf);
    armas_mult(ZERO, &At, ONE, &C, &V, ARMAS_TRANSB, &conf);

    if (verbose > 2 && N < 10) {
        printf("D:\n");
        armas_printf(stdout, "%6.3f", &D);
        printf("At:\n");
        armas_printf(stdout, "%6.3f", &At);
    }
    // compute ||U.T*A*V - S|| (D is column vector, sD is row vector)
    armas_diag(&sD, &At, 0);
    abs_minus(&sD, &D);
    nrm = armas_mnorm(&sD, ARMAS_NORM_ONE, &conf) / nrm_A;
    ...
    // compute: ||I - U.T*U||
    armas_mult(ZERO, &C, ONE, &U, &U, ARMAS_TRANSA, &conf);
    armas_diag(&sD, &C, 0);
    armas_madd(&sD, -ONE, 0, &conf);
    ...
    // compute ||I - V*V.T||_1
    armas_mult(ZERO, &C, ONE, &V, &V, ARMAS_TRANSA, &conf);
    armas_madd(&sD, -ONE, 0, &conf);
