...
int test_lss(int M, int N, int K, int lb, int verbose)
{
    ...
    // compute: B0 = A0*X0
    armas_mult(0.0, &B0, 1.0, &A0, &X0, ARMAS_NONE, &conf);

    // factor
    armas_qrfactor(&A0, &tau0, &conf);

    // solve B0 = A.-1*B0
    armas_qrsolve(&B0, &A0, &tau0, ARMAS_NONE, &conf);

    // X0 = X0 - A.-1*B0
    armas_submatrix(&X, &B0, 0, 0, N, K);
...
int test_min(int M, int N, int K, int lb, int verbose)
{
    ...
    armas_qrsolve(&X0, &A0, &tau0, ARMAS_TRANS, &conf);

    // B = B - A.T*X
    armas_submatrix(&B, &B0, 0, 0, N, K);
    nrm0 = armas_mnorm(&B0, ARMAS_NORM_ONE, &conf);
    armas_mult(1.0, &B, -1.0, &A1, &X0, ARMAS_TRANSA, &conf);
