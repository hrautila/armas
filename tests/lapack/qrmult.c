...
int test_mult_identity(int M, int N, int lb, int verbose)
{
    ...
    armas_x_mcopy(&C1, &C, 0, &conf);

    // factorize
    armas_x_qrfactor(&A0, &tau0, &conf);

    armas_x_qrmult(&C, &A0, &tau0, ARMAS_LEFT, &conf);
    armas_x_qrmult(&C, &A0, &tau0, ARMAS_LEFT | ARMAS_TRANS, &conf);
...
int test_mult_left(int M, int N, int lb, int verbose)
{
    ...
    // C is first N columns of identity matrix
    armas_x_set_values(&C0, unitrand, 0);
    ...
    // compute C0 = Q.T*Q*C0
    armas_x_qrmult(&C0, &A0, &tau0, ARMAS_LEFT, &conf);
    armas_x_qrmult(&C0, &A0, &tau0, ARMAS_LEFT | ARMAS_TRANS, &conf);
...
int test_mult_right(int M, int N, int lb, int verbose)
{
    ...
    // compute C0 = C0*Q.T*Q
    armas_x_qrmult(&C0, &A0, &tau0, ARMAS_RIGHT | ARMAS_TRANS, &conf);
    armas_x_qrmult(&C0, &A0, &tau0, ARMAS_RIGHT, &conf);
