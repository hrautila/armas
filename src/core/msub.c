...
/**
 * @brief Element wise substraction of A = A - B
 *
 * @param[in,out] A
 *    On entry, first input matrix. On exit result matrix.
 * @param[in] B
 *    Second input matrix. If B is 1x1 then operation equals to subtracting constant
...
int armas_x_sub_elems(armas_x_dense_t *A, const armas_x_dense_t *B, int flags)
{
  ...
  case ARMAS_LOWER:
    if (flags & ARMAS_ABS) {
      __msub_lower_abs(A, B, A->rows, A->cols);
    } else {
      __msub_lower(A, B, A->rows, A->cols);
    }
    break;
  case ARMAS_UPPER:
    if (flags & ARMAS_ABS) {
      __msub_upper_abs(A, B, A->rows, A->cols);
    } else {
      __msub_upper(A, B, A->rows, A->cols);
    }
    break;
  default:
    if (flags & ARMAS_ABS) {
      __msub_abs(A, B, A->rows, A->cols, flags);
    } else {
      __msub(A, B, A->rows, A->cols, flags);
    }
    break;
  }
