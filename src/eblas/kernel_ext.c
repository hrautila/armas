...
// this file provides following type independent functions
    ...
    defined(armas_ext_mult_kernel_nc)
    ...
    cache_t *cache)
{
    ...
    for (kp = 0; kp < P; kp += cache->KB) {
        ...
        armas_make(&Acpy, nK, nI, cache->ab_step, cache->Acpy);
        armas_make(&Bcpy, nK, nJ, cache->ab_step, cache->Bcpy);

        if (flags & (ARMAS_TRANSB|ARMAS_CTRANSB)) {
            armas_submatrix_unsafe(&Bc, B, 0, kp, nJ, nK);
            CPBLK_TRANS(&Bcpy, &Bc, nJ, nK, flags);
        } else {
            armas_submatrix_unsafe(&Bc, B, kp, 0, nK, nJ);
            CPBLK(&Bcpy, &Bc, nK, nJ, flags);
        }

        if (flags & (ARMAS_TRANSA|ARMAS_CTRANSA)) {
            armas_submatrix_unsafe(&Ac, A, kp, 0, nK, nI);
            CPBLK(&Acpy, &Ac, nK, nI, flags);
        } else {
            armas_submatrix_unsafe(&Ac, A, 0, kp, nI, nK);
            CPBLK_TRANS(&Acpy, &Ac, nI, nK, flags);
        }

        armas_ext_mult_inner(C, dC, alpha, &Acpy, &Bcpy, cache->rb);
    }
}

// error free update: C + dC += (A + dA)*B   C is nR,nJ A panel is nR,nP and B panel is nP,nJ
// C + dC += A*B; dC += dA*B
void armas_ext_panel_dA_unsafe(
    armas_dense_t *C,
    armas_dense_t *dC,
    DTYPE alpha,
    const armas_dense_t *A,
    const armas_dense_t *dA,
    const armas_dense_t *B,
    int flags,
    cache_t *cache)
{
    int kp;
    armas_dense_t Acpy, Bcpy, Bc, Ac;
    int nJ = C->cols;
    int nI = C->rows;
    ...
    for (kp = 0; kp < P; kp += cache->KB) {
        ...
        // update dC += dA*B
        if (flags & (ARMAS_TRANSA|ARMAS_CTRANSA)) {
            armas_submatrix_unsafe(&Ac, dA, kp, 0, nK, nI);
            CPBLK(&Acpy, &Ac, nK, nI, flags);
        } else {
            armas_submatrix_unsafe(&Ac, dA, 0, kp, nI, nK);
            CPBLK_TRANS(&Acpy, &Ac, nI, nK, flags);
        }

        armas_mult_kernel_inner(dC, alpha, &Acpy, &Bcpy, cache->rb);
    }
}
...
// C + dC += A*B; dC += A*dB
void armas_ext_panel_dB_unsafe(
    armas_dense_t *C, 
    armas_dense_t *dC,
    DTYPE alpha,
    const armas_dense_t *A,
    const armas_dense_t *B,
    const armas_dense_t *dB,
    int flags,
    cache_t *cache)
{
    int kp;
    armas_dense_t Ac, Bc, Acpy, Bcpy;
    int nJ = C->cols;
    int nI = C->rows;
    int P = (flags & (ARMAS_TRANSA|ARMAS_CTRANSA)) != 0 ? A->rows : A->cols;
    ...
    for (kp = 0; kp < P; kp += cache->KB) {
        ...
        if (flags & (ARMAS_TRANSB|ARMAS_CTRANSB)) {
            armas_submatrix_unsafe(&Bc, dB, 0, kp, nJ, nK);
            CPBLK_TRANS(&Bcpy, &Bc, nJ, nK, flags);
        } else {
            armas_submatrix_unsafe(&Bc, dB, kp, 0, nK, nJ);
            CPBLK(&Bcpy, &Bc, nK, nJ, flags);
        }
        // dC += A*dB
        armas_mult_kernel_inner(dC, alpha, &Acpy, &Bcpy, cache->rb);
    ...
    cache_t *cache)
{
    ...
    armas_dense_t Ca, Ac, Bc, Acpy, Bcpy, dC;
    ...
    for (jp = 0; jp < C->cols; jp += cache->NB) {
        ...
        for (ip = 0; ip < C->rows; ip += cache->MB) {
            ...
            armas_make(&dC, nI, nJ, cache->ab_step, cache->dC);
            armas_submatrix_unsafe(&Ca, C, ip, jp, nI, nJ);
            // compute C0 + dC = beta*C
            if (beta != ONE)
                armas_ext_scale_unsafe(&Ca, &dC, beta, &Ca);
            else
                armas_scale_unsafe(&dC, ZERO);
            ...
            for (kp = 0; kp < P; kp += cache->KB) {
                ...
                armas_ext_mult_inner(&Ca, &dC, alpha, &Acpy, &Bcpy, cache->rb);
            }
            // merge back to target
            armas_merge2_unsafe(&Ca, &Ca, &dC);
        }
    }
    return 0;
}

int armas_ext_mult_kernel_nc(
    armas_dense_t *C,
    DTYPE alpha,
    const armas_dense_t *A,
    const armas_dense_t *B,
    ...
    cache_t *cache)
{
    return armas_ext_mult_kernel(ONE, C, alpha, A, B, flags, cache);
}
