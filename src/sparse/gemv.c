...
// CSC storage
static
int csc_mvmult(DTYPE beta, DTYPE *y, DTYPE alpha, const armas_x_sparse_t *A, const DTYPE *x, int flags)
{
...
static
int csr_mvmult(DTYPE beta, DTYPE *y, DTYPE alpha, const armas_x_sparse_t *A, const DTYPE *x, int flags)
{
    DTYPE ax, *Ae = A->elems.v;
    if (flags == 0) {
        // y + alpha*A*x
