...
int csr_illtz_lower(armas_x_sparse_t * A)
{
    ...
    for (i = 0; i < A->rows; i++) {
        ...
        for (k = i + 1; k < A->rows; k++) {
            ...
            for (p = p1 + 1;
                 armassp_x_at(A, p) <= k && p < armassp_x_index(A, k + 1);
                 p++) {
                // here j <= k
                j = armassp_x_at(A, p);
...
int csr_illtz_upper(armas_x_sparse_t * A)
{
    ...
    for (i = 0; i < A->rows; i++) {
        ...
        for (p = p0 + 1; p < armassp_x_index(A, i + 1); p++) {
            ...
            j = armassp_x_at(A, p);
            // find diagonal
            for (p2 = armassp_x_index(A, j);
                 armassp_x_at(A, p2) < j && p2 < armassp_x_index(A, j + 1);
                 p2++);
            // update values on and above diagonal
            for (p1 = p0 + 1;
                 p1 < armassp_x_index(A, i + 1)
                 && p2 < armassp_x_index(A, j + 1);) {
...
static
int precond_icholz(armas_x_dense_t * z,
...
static
int precond_icholz_partial(armas_x_dense_t * z,
                           const armassp_x_precond_t * P,
