...
#if defined(armassp_pcgrad_w) && defined(armassp_pcgrad)
 ...
 * @ingroup sparse
 */
int armassp_pcgrad_w(armas_dense_t * x,
...
/**
 * @brief Solve x = A^-1*b by conjugate gradient method with preconditioner.
 * @see armassp_pcgrad_w
 * @ingroup sparse
 */
int armassp_pcgrad(armas_dense_t * x,
                     const armas_sparse_t * A,
                     const armas_dense_t * b,
                     armassp_precond_t * M, int flags, armas_conf_t * cf)
{
    int stat;
    ...
    // get working size
    if (armassp_pcgrad_w(x, A, b, M, flags, &W, cf) < 0) {
        return -1;
    }
    ...
    stat = armassp_pcgrad_w(x, A, b, M, flags, &W, cf);
    armas_wrelease(&W);
    return stat;
}
#else
