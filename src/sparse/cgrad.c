 ...
 *    Initial vector b
 * \param [in] flags
 *    Indicator bits. If ARMAS_LOWER (ARMAS_UPPER) is set then A is a lowet (upper) 
 *    triangular matrix.
 * \param [in] W
 *    Workspace for intermediate results. If W.bytes is zero then workspace size is
 *    computed and control returned immediately to caller.
 * \param [in,out] cf
