...
#if defined(armas_x_mult_kernel_inner) && \
    defined(armas_x_mult_kernel) && defined(armas_x_mult_kernel_nc)
...
void armas_x_mult_kernel_inner(
    ...
    for (jp = 0; jp < C->cols; jp += cache->NB) {
        ...
        for (kp = 0; kp < P; kp += cache->KB) {
            ...
            for (ip = 0; ip < C->rows; ip += cache->MB) {
                ...
                armas_x_mult_kernel_inner(&Ca,alpha, &Acpy, &Bcpy, cache->rb);
            }
        }
    }
    return 0;
