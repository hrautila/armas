    ...
    int flags)
{
    ...
    register int j;
    DTYPE scal;
    armas_dense_t B2, a1, b1;

    armas_row_unsafe(&b1, B, A->cols-1);
    scal = unit ? alpha : alpha/armas_get_unsafe(A, A->cols-1, A->cols-1);
    if (scal != ONE)
        armas_scale_unsafe(&b1, scal);

    for (j = A->cols-2; j >= 0; j--) {
        armas_submatrix_unsafe(&B2, B, j+1, 0, B->rows-1-j, B->cols);
        armas_row_unsafe(&b1, B, j);
        armas_submatrix_unsafe(&a1, A, j, j+1, 1, A->cols-1-j);
        // b1 = (alpha*b1 - B2.T*a1) / a11
        armas_mvmult_unsafe(alpha, &b1, -ONE, &B2, &a1, ARMAS_TRANS);
        if (!unit)
            armas_scale_unsafe(&b1, ONE/armas_get_unsafe(A, j, j));
    }
}
    ...
    int flags)
{
    ...
    DTYPE scal;
    register int j;
    armas_dense_t B0, a1, b1;

    armas_row_unsafe(&b1, B, 0);
    scal = unit ? alpha : alpha/armas_get_unsafe(A, 0, 0);
    if (scal != ONE)
        armas_scale_unsafe(&b1, scal);

    for (j = 1; j < A->cols; j++) {
        armas_submatrix_unsafe(&B0, B, 0, 0, j, B->cols);
        // j'th column
        armas_row_unsafe(&b1, B, j);
        armas_submatrix_unsafe(&a1, A, 0, j, j, 1);
        // b1 = (alpha*b1 - B0*a1) / a11
        armas_mvmult_unsafe(alpha, &b1, -ONE, &B0, &a1, ARMAS_TRANS);
        if (!unit)
            armas_scale_unsafe(&b1, ONE/armas_get_unsafe(A, j, j));
    }
}
    ...
    int flags)
{
    ...
    for (j = 1; j < A->cols; j++) {
        ...
        // j'th column
        armas_row_unsafe(&b1, B, j);
        armas_submatrix_unsafe(&a1, A, j, 0, 1, j);
        // b1 = (alpha*b1 - B0*a1) / a11
        armas_mvmult_unsafe(alpha, &b1, -ONE, &B0, &a1, ARMAS_TRANS);
        if (!unit)
            armas_scale_unsafe(&b1, ONE/armas_get_unsafe(A, j, j));
    ...
    int flags)
{
    ...
    for (j = A->cols-2; j >= 0; j--) {
        armas_submatrix_unsafe(&B2, B, j+1, 0, A->cols-1-j, B->cols);
        armas_row_unsafe(&b1, B, j);
        armas_submatrix_unsafe(&a1, A, j+1, j, A->cols-1-j, 1);
        // b1 = (alpha*b1 - B2.T*a1) / a11
        armas_mvmult_unsafe(alpha, &b1, -ONE, &B2, &a1, ARMAS_TRANS);
        if (!unit)
            armas_scale_unsafe(&b1, ONE/armas_get_unsafe(A, j, j));
    ...
    int flags)
{
    ...
    armas_column_unsafe(&b1, B, 0);
    scal = unit ? alpha : alpha/armas_get_unsafe(A, 0, 0);
    if (scal != ONE)
        armas_scale_unsafe(&b1, scal);

    for (j = 1; j < A->cols; j++) {
        armas_submatrix_unsafe(&B0, B, 0, 0, B->rows, j);
        // j'th column
        armas_column_unsafe(&b1, B, j);
        armas_submatrix_unsafe(&a1, A, 0, j, j, 1);
        // b1 = (alpha*b1 - B0*a1) / a11
        armas_mvmult_unsafe(alpha, &b1, -ONE, &B0, &a1, 0);
        if (!unit)
            armas_scale_unsafe(&b1, ONE/armas_get_unsafe(A, j, j));
    }
}
    ...
    int flags)
{
    ...
    armas_column_unsafe(&b1, B, A->cols-1);
    scal = unit ? alpha : alpha/armas_get_unsafe(A, A->cols-1, A->cols-1);
    if (scal != ONE)
        armas_scale_unsafe(&b1, scal);
    ...
    for (j = A->cols-2; j >= 0; j--) {
        // j'th column
        armas_submatrix_unsafe(&B2, B, 0, j+1, B->rows, A->cols-1-j);
        armas_column_unsafe(&b1, B, j);
        armas_submatrix_unsafe(&a1, A, j, j+1, 1, A->cols-1-j);
        // b1 = (alpha*b1 - B0*a1) / a11
        armas_mvmult_unsafe(alpha, &b1, -ONE, &B2, &a1, 0);
        if (!unit)
            armas_scale_unsafe(&b1, ONE/armas_get_unsafe(A, j, j));
    }
}
