...
void rational_forward(DTYPE * val, DTYPE * dval,
                      armas_x_dense_t * Z, armas_x_dense_t * delta,
                      int start, int end)
{
    int i;
    DTYPE dj, zj, tval;
 ...
 */
static
int trdsec_root(DTYPE * lambda, armas_x_dense_t * D, armas_x_dense_t * Z,
                armas_x_dense_t * delta, int index, DTYPE rho)
{
 ...
 */
static
void update_vec_delta(DTYPE * vk, armas_x_dense_t * d,
                      armas_x_dense_t * delta, int index, DTYPE rho)
{
 ...
 */
static inline
void trdsec_update_vec_delta(armas_x_dense_t * z, armas_x_dense_t * Q,
 ...
 */
static
void trdsec_eigenvec_delta(armas_x_dense_t * qi, armas_x_dense_t * dl,
                           armas_x_dense_t * z, armas_conf_t *cf)
{
    DTYPE dk, zk, t, s;
    int k;
    ...
    s = armas_x_nrm2(qi, cf);
    armas_x_scale(qi, ONE/s, cf);
}
 ...
 */
static
void trdsec_eigenbuild_inplace(armas_x_dense_t * Q, armas_x_dense_t * z,
                               armas_conf_t *cf)
{
    ...
    for (k = 0; k < armas_x_size(z); k++) {
        armas_x_column(&qi, Q, k);
        t = armas_x_nrm2(&qi, cf);
        armas_x_scale(&qi, ONE/t, cf);
    }
}
 ...
 * @ingroup lapack
 */
int armas_x_trdsec_solve_vec(armas_x_dense_t * y, armas_x_dense_t * v,
                             armas_x_dense_t * Qd, armas_x_dense_t * d,
                             armas_x_dense_t * z, DTYPE rho,
                             armas_conf_t * conf)
{
    armas_x_dense_t delta;
    DTYPE dlam;
 ...
 * @ingroup lapack
 */
int armas_x_trdsec_eigen(armas_x_dense_t * Q, armas_x_dense_t * v,
                         armas_x_dense_t * Qd, armas_conf_t * conf)
{
    if (!conf)
        conf = armas_conf_default();
 ...
 */
static inline
    void __update_vec(DTYPE * vk, armas_x_dense_t * d, armas_x_dense_t * dl,
                      int index, DTYPE rho)
{
    DTYPE n0, n1, dn, dk, val, p0, p1;
    int k, N;
