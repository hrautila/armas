...
{
  // W = C1.T
  IFERROR(armas_x_scale_plus(__ZERO, W, __ONE, C1, ARMAS_TRANSB, conf));
  // W = C1.T*Y1 = W*Y1
  IFERROR(armas_x_mult_trm(W, __ONE, Y1, ARMAS_UPPER|ARMAS_UNIT|ARMAS_RIGHT, conf));
  // W = W + C2.T*Y2
  IFERROR(armas_x_mult(__ONE, W, __ONE, C2, Y2, ARMAS_TRANSA, conf));
  // here: W = C.T*Y
  ...
  if (! transpose)
    bits |= ARMAS_TRANSA;
  // W = W*T or W.T*T
  IFERROR(armas_x_mult_trm(W, __ONE, T, bits, conf));
  // here: W == C.T*Y*T or C.T*Y*T.T

  // C2 = C2 - Y2*W.T
  IFERROR(armas_x_mult(__ONE, C2, -__ONE, Y2, W, ARMAS_TRANSB, conf));
  // W = Y1*W.T ==> W.T = W*Y1.T
  IFERROR(armas_x_mult_trm(W, __ONE, Y1, ARMAS_UPPER|ARMAS_UNIT|ARMAS_TRANSA|ARMAS_RIGHT, conf));
  // C1 = C1 - W.T
  IFERROR(armas_x_scale_plus(__ONE, C1, -__ONE, W, ARMAS_TRANSB, conf));
...
{
  // W = C1
  IFERROR(armas_x_scale_plus(__ZERO, W, __ONE, C1, ARMAS_NONE, conf));
  // W = C1*Y1 = W*Y1
  IFERROR(armas_x_mult_trm(W, __ONE, Y1, ARMAS_UPPER|ARMAS_UNIT|ARMAS_RIGHT, conf));
  // W = W + C2*Y2
  IFERROR(armas_x_mult(__ONE, W, __ONE, C2, Y2, ARMAS_NONE, conf));
  // here: W = C*Y
  ...
  // C2 = C2 - W*Y2.T
  IFERROR(armas_x_mult(__ONE, C2, -__ONE, W, Y2, ARMAS_TRANSB, conf));
  // C1 = C1 - W*Y1*T
  //  W = W*Y1.T
  IFERROR(armas_x_mult_trm(W, __ONE, Y1, ARMAS_UPPER|ARMAS_UNIT|ARMAS_TRANSA|ARMAS_RIGHT, conf));
  // C1 = C1 - W
  IFERROR(armas_x_scale_plus(__ONE, C1, -__ONE, W, ARMAS_NONE, conf));
                         ...
                         armas_conf_t *conf)
{
  ...
  while (ATL.rows > 0 && ATL.cols > 0) {
    ...
    if (tauval != 0.0) {
      ...
      armas_x_axpby(__ZERO, &t21, __ONE, &a12, conf);
      armas_x_mvmult(-tauval, &t21, -tauval, &A02, &a01, ARMAS_TRANSA, conf);
      // t21 := T22*t21
