 ...
 *
 * Applying the convergence criteria. In section 4 we presented four convergence
 * criteria. Since applying the convergence criteria costs approximately as many
 * floating point* operations (O(n)) as performing a QR sweep, it is important
 * to test criteria only when they are likely to be satisfied. Our decision is
 * based on the following empirical observation: When chasing the bulge down
 * (up) the bottom most (topmost) entry s(n-1) (s(0))* often tends to converge
 * to the smallest singular value, with e(n − 2) (e(0)) tending to zero fastest
 * of all offdiagonal entries. Therefore, when chasing the bulge down, we expect
 * convergence criteria 1a an d 2a to be successful, and possibly 1b but only
 * for the bottom most entry e(n−2) . Criteria 2b an d 1b for the other off
 * diagonal entries are not as likely to succeed. Conversely, when chasing the
 * bulge up, we only apply convergence criteria 1b, 2b and 1a for e(0). One
 * advantage of this scheme is that testing 2a (for e(n−2) , and if the test
 * succeeds, for e(n-3) too) costs only a few more operations after testing 1a,
 * since they share the same recurrence from (4.3). Similarly, 2b (for e(0) ,
 * and if the test succeeds, for e(1) too) is very cheap after applying 1b.
 */
                         ...
                         armas_conf_t * conf)
{
    ...
    if (conf->optflags & ARMAS_OABSTOL) {
        threshold = maxit * SAFEMIN;
        if (tol * shigh < threshold)
            threshold = tol * shigh;
    } else {
        threshold = tol * slow;
        if (maxit * SAFEMIN < threshold)
            threshold = maxit * SAFEMIN;
    }
    ...
    for (work = 1, n = 0; work && maxit > 0 && iq > 0; maxit--, n++) {
        ...
        // compute shift
        if (!abstol && (N * tol * (slow / shigh)) <= max(EPS, 0.01 * tol)) {
            zero = 1;
        } else {
            if (forwards) {
                d0 = ABS(armas_get_at_unsafe(D, ip));
