 ...
 */
static
int __find_bkpivot_lower(armas_x_dense_t *A, int *nr, int *np, armas_conf_t *conf)
{
 ...
 */
int __unblk_bkfactor_lower(armas_x_dense_t *A, armas_x_dense_t *W,
 ...
 */
static
int __build_bkpivot_lower(armas_x_dense_t *AL, armas_x_dense_t *AR,
                          ...
                          int k, int *nr, int *np, armas_conf_t *conf)
{
  ...
  if (k > 0) {
    ...
    armas_x_mvmult(__ONE, &wk, -__ONE, AL, &wrow, ARMAS_NONE, conf);
  }
  if (AR->rows == 1) {
    *nr = 0; *np = 1;
    return 0;
  ...
  r = armas_x_iamax(&rcol, conf) + 1;
  rmax = __ABS(armas_x_get(&rcol, r-1, 0));
  if (amax >= bkALPHA*rmax) {
    // no pivoting, 1x1 diagonal
    *nr = 0; *np = 1;
    return 0;
  ...
  if (k > 0) {
    ...
    armas_x_mvmult(__ONE, &wkp1, -__ONE, AL, &wrow, ARMAS_NONE, conf);
  }
 ...
 */
static
int __unblk_bkbounded_lower(armas_x_dense_t *A, armas_x_dense_t *W,
...
int __blk_bkfactor_lower(armas_x_dense_t *A, armas_x_dense_t *W,
...
int __unblk_bksolve_lower(armas_x_dense_t *B, armas_x_dense_t *A,
                          armas_pivot_t *P, int phase, armas_conf_t *conf)
{
  ...
  while (Aref->cols > 0) {
    ...
    case 2:
      if (np == 1) {
        armas_x_mvmult(__ONE, &b1, -__ONE, &B2, &a21, ARMAS_TRANSA, conf);
        if (pr != nc) {
          // swap rows on bottom part of B
          __merge2x1(&Bx, &b1, &B2);
