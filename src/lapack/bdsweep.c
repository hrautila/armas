...
#if defined(armas_bd_qrsweep) && defined(armas_bd_qlsweep)
                       ...
                       DTYPE f0, DTYPE g0, int saves)
{
    ...
    d2 = e2 = ZERO;
    d1 = armas_get_at_unsafe(D, 0);
    e1 = armas_get_at_unsafe(E, 0);
    f = f0;
    g = g0;
    for (k = 0; k < N - 1; k++) {
        d2 = armas_get_at_unsafe(D, k + 1);
        ...
        armas_gvrotate(&f, &e1, cosr, sinr, d1, e1);
        // g = sinr*d[i+1]; d[i+1] = cosr*d[i+];
        armas_gvrotate(&g, &d2, cosr, sinr, 0.0, d2);
        armas_gvcompute(&cosl, &sinl, &r, f, g);
        // d[i] = r
        d1 = r;
        // f = cosl*e[i] + sinl*d[i+1]; d[i+1] = cosl*d[i+1] - sinl*e[i];
        armas_gvrotate(&f, &d2, cosl, sinl, e1, d2);
        if (k < N - 2) {
            e2 = armas_get_at_unsafe(E, k + 1);
            // g = sinl*e[i+1]; e[i+1] = cosl*e[i+1];
            armas_gvrotate(&g, &e2, cosl, sinl, 0.0, e2);
            armas_set_at_unsafe(E, k + 1, e2);
        }

        armas_set_at_unsafe(D, k, d1);
        armas_set_at_unsafe(D, k + 1, d2);
        d1 = d2;
        e1 = e2;
                      ...
                      armas_dense_t * Cl, armas_dense_t * Sl, int saves)
{
    ...
    cosl = ONE;
    for (k = 0; k < N - 1; k++) {
        e1 = armas_get_at_unsafe(E, k);
        d2 = armas_get_at_unsafe(D, k + 1);
        armas_gvcompute(&cosr, &sinr, &r, d1 * cosr, e1);
        if (k > 0)
            armas_set_at_unsafe(E, k - 1, sinl * r);
        armas_gvcompute(&cosl, &sinl, &r, cosl * r, sinr * d2);
    ...
    d2 = cosr * d2;
    armas_set_at_unsafe(D, N - 1, d2 * cosl);
    armas_set_at_unsafe(E, N - 2, d2 * sinl);
    return N - 1;
}
...
{
    DTYPE d1, e1, d2, e2, f, g, cosr, cosl, sinr, sinl, r;
    int k, n, N = armas_size(D);

    d1 = e2 = ZERO;
    d1 = armas_get_at_unsafe(D, N - 1);
    e1 = armas_get_at_unsafe(E, N - 2);
    f = f0;
    g = g0;
    for (n = 0, k = N - 1; k > 0; k--, n++) {
        ...
        armas_gvrotate(&f, &d2, cosl, sinl, e1, d2);
        if (k > 1) {
            e2 = armas_get_at_unsafe(E, k - 2);
            //g  = sinl*e2;  e2 = cosl*e2;
            armas_gvrotate(&g, &e2, cosl, sinl, 0.0, e2);
            armas_set_at_unsafe(E, k - 2, e2);
        }
        ...
        if (saves) {
            ...
            armas_set_at_unsafe(Cl, k - 1, cosl);
            armas_set_at_unsafe(Sl, k - 1, -sinl);
        }
    }
    armas_set_at_unsafe(E, 0, f);
    return N - 1;
}
                      ...
                      armas_dense_t * Cl, armas_dense_t * Sl, int saves)
{
    ...
    for (n = 0, k = N - 1; k > 0; k--, n++) {
        ...
        armas_gvcompute(&cosr, &sinr, &r, d1 * cosr, e1);
        if (k < N - 1)
            armas_set_at_unsafe(E, k, sinl * r);
        armas_gvcompute(&cosl, &sinl, &r, cosl * r, sinr * d2);
        armas_set_at_unsafe(D, k, r);
        d1 = d2;
        if (saves) {
    ...
    d2 = cosr * armas_get_at_unsafe(D, 0);
    armas_set_at_unsafe(D, 0, d2 * cosl);
    armas_set_at_unsafe(E, 0, d2 * sinl);
    return N - 1;
}
