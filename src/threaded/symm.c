...
void *threaded_compute_block(void *argptr)
{
    ...
    armas_x_mult_sym_unsafe(blas->beta, &C, blas->alpha, &A, &B, blas->flags, args->index, &cache);
...
static
int threaded_mult_sym_recursive(int thread, int nthreads, struct armas_ac_blas3 *blas, armas_conf_t *cf)
{
    int first_col, last_col, first_row, last_row, err, index;
    pthread_t th;
...
int armas_ac_threaded_mult_sym(struct armas_ac_blas3 *args, armas_conf_t *cf, struct armas_threaded_conf *acf)
{
