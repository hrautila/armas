...
#if defined(trmvf) || defined(cblas_trmv)
#define ARMAS_PROVIDES 1
#endif
// this file requires external public functions
#if defined(armas_mvmult_trm)
...
#if defined(blas_trmvf)
void blas_trmvf(char *uplo, char *trans, char *diag, int *n, DTYPE * A,
                int *lda, DTYPE * X, int *incx)
{
    ...
    armas_mvmult_trm(&x, ONE, &a, flags, conf);
}
#endif

#if defined(cblas_trmv)
                ...
                int N, DTYPE alpha, DTYPE * A, int lda, DTYPE * X, int incx)
{
    ...
    armas_mvmult_trm(&x, alpha, &Aa, flags, conf);
}
#endif
