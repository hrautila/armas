...
// don't use six column versions; 
#define HAVE_6COL 0

// update 1x6 block of C; one row, six columns
static inline
void __mult1c6(double *c0, double *c1, double *c2, double *c3, double *c4, double *c5,
               const double *a, const double *b0, const double *b1, const double *b2,
               const double *b3, const double *b4, const double *b5, double alpha, int nR)
{
  register int k;
  ...
  for (k = 0; k < nR-3; k += 4) {
    ...
    y4 += _mm256_mul_pd(A0, mm_load_B(&b4[k]));
    y5 += _mm256_mul_pd(A0, mm_load_B(&b5[k]));
  }
  if (k == nR)
    goto update;
...
// update 2x6 block of C; two rows, six columns
static inline
void __mult2c6(double *c0, double *c1, double *c2, double *c3, double *c4, double *c5,
               const double *a0, const double *a1, const double *b0, const double *b1, 
               const double *b2, const double *b3, const double *b4, const double *b5,
               double alpha, int nR)
{
  register int k;
  ...
  for (k = 0; k < nR-3; k += 4) {
    ...
    y6  += _mm256_mul_pd(A1, mm_load_B(&b0[k]));
    y7  += _mm256_mul_pd(A1, mm_load_B(&b1[k]));
    y8  += _mm256_mul_pd(A1, mm_load_B(&b2[k]));
    y9  += _mm256_mul_pd(A1, mm_load_B(&b3[k]));
    y10 += _mm256_mul_pd(A1, mm_load_B(&b4[k]));
    y11 += _mm256_mul_pd(A1, mm_load_B(&b5[k]));
