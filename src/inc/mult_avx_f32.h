               ...
               const float *b2, const float *b3, float alpha, int nR)
{
  ...
  for (k = 0; k < nR-7; k += 8) {
    A0 = mm_load_A(&a[k]);
    y0 += _mm256_mul_ps(A0, mm_load_B(&b0[k]));
    y1 += _mm256_mul_ps(A0, mm_load_B(&b1[k]));
    y2 += _mm256_mul_ps(A0, mm_load_B(&b2[k]));
    y3 += _mm256_mul_ps(A0, mm_load_B(&b3[k]));
  }
  if (k == nR)
    goto update;
               ...
               float alpha, int nR)
{
  ...
  for (k = 0; k < nR-7; k += 8) {
    ...
    y4 += _mm256_mul_ps(A1, mm_load_B(&b0[k]));
    y5 += _mm256_mul_ps(A1, mm_load_B(&b1[k]));
    y6 += _mm256_mul_ps(A1, mm_load_B(&b2[k]));
    y7 += _mm256_mul_ps(A1, mm_load_B(&b3[k]));
               ...
               float alpha, int nR)
{
  ...
  for (k = 0; k < nR-7; k += 8) {
    ...
    y0 += _mm256_mul_ps(A0, mm_load_B(&b0[k]));
    y1 += _mm256_mul_ps(A0, mm_load_B(&b1[k]));
    y2 += _mm256_mul_ps(A1, mm_load_B(&b0[k]));
    y3 += _mm256_mul_ps(A1, mm_load_B(&b1[k]));
  }
  if (k == nR)
    goto update;
...
void __mult1c1(float *c, const float *a, const float *b, float alpha, int nR)
{
  ...
  for (k = 0; k < nR-7; k += 8) {
    A  = mm_load_A(&a[k]);
    y0 += _mm256_mul_ps(A, mm_load_B(&b[k]));
  }
  if (k == nR)
    goto update;
               ...
               const float *b0, float alpha, int nR)
{
  ...
  for (k = 0; k < nR-7; k += 8) {
    B0 = mm_load_B(&b0[k]);
    y0 += _mm256_mul_ps(B0, mm_load_A(&a0[k]));
    y1 += _mm256_mul_ps(B0, mm_load_A(&a1[k]));
    y2 += _mm256_mul_ps(B0, mm_load_A(&a2[k]));
    y3 += _mm256_mul_ps(B0, mm_load_A(&a3[k]));
  }
  if (k == nR)
    goto update;
               ...
               const float *b0, const float *b1, float alpha, int nR)
{
  ...
  for (k = 0; k < nR-7; k += 8) {
    ...
    y2 += _mm256_mul_ps(B0, mm_load_A(&a2[k]));
    y3 += _mm256_mul_ps(B0, mm_load_A(&a3[k]));
    y4 += _mm256_mul_ps(B1, mm_load_A(&a0[k]));
    y5 += _mm256_mul_ps(B1, mm_load_A(&a1[k]));
    y6 += _mm256_mul_ps(B1, mm_load_A(&a2[k]));
    y7 += _mm256_mul_ps(B1, mm_load_A(&a3[k]));
